package com.javarush.task.task30.task3007;

/* 
Найдем число 2 в максимальной степени
*/
public class Solution {
    public static void main(String[] args) {
        System.out.println(maxPowerOf2(140_000));   //131072
        System.out.println(maxPowerOf2(1026));      //1024
        System.out.println(maxPowerOf2(17));        //16
    }

    public static int maxPowerOf2(int x) {
        // the simpliest solution, but Validator doesn't validate it
        // return 1 << Integer.toBinaryString(x).length()-1;

        // then consider the solution of task 3013 to solve this task

        x |= ( x >> 1 );
        x |= ( x >> 2 );
        x |= ( x >> 4 );
        x |= ( x >> 8 );
        x |= ( x >> 16 );
        return x & ~(x >> 1); // find the right-most 1bit, which is the solution of task.
    }
}